...
while true
  ...
  if user_input == '1'
    ...
    new_list.each do |item|
      unless new_array.include? "#{"[X]"} : #{item}" && "#{"[ ]"} : #{item}"
       new_array.push("#{"[ ]"} : #{item}")
     end
    end
    puts new_list
    new_array.map.with_index do |l,i|
      puts "(#{(i.next).to_s}) : #{l}"
    end
    next
  elsif user_input == '3'
    ...
    new_array.delete_at((remove_entry)-1)
    next
  elsif user_input == '6'
    ...
    new_array[(replace)-1] = new_task
    next
  elsif user_input == '7'
    puts "Which task did you complete?"
    toggle_task = Integer(gets.chomp)
    new_array[(toggle_task)-1] = "#{"[X]"} : #{new_list[(toggle_task)-1]}"
    next
