...
#Second Answer
#Check the if for appending or pushing
class Raindrops
  attr_accessor :number
  attr_accessor :new_list
  def initialize(number)
  ...
  def convert
    new_list = ""
    new_list << "Pling" if number % 3 == 0
    new_list << "Plang" if number % 5 == 0
    new_list << "Plong" if number % 7 == 0
    new_list = number if new_list.empty?
    print new_list
  end
end
check = Raindrops.new(105)
